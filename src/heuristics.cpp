...
void ISCore(Cnode* root, unsigned long num_processors, bool sequentialPart){//number of processors here assumed to the same as tree'size
...
Ctree* BuildQtree(Ctree* tree){//Qtree is for makespan side, so do not use it for space side
    Cnode* root=tree->GetRoot();
    root->BreakEdge();
...
bool increaseMS(Ctree* tree, Ctree* Qtree, Cnode* & smallestNode, int * chstart,int * childrenID, double memory_size, bool CheckMemory){
    ...
    for (; iter!=subtrees->end(); ++iter) {
        ...
        if (tree->GetNode(currentNode->GetothersideID())->IsBorken()==true) {//this subtree has not been merged yet
            children=currentNode->GetChildren();
                ...
                if (children->empty()) {
                    ...
                    if (children->size()==2) {
                        ...
                        if (currentNode->GetMSCost(true, false)==LargestNode->GetMSCost(true,false)) {
                            ...
                            }else{increase=-currentNode->GetEW()/BANDWIDTH+secondLargest->GetMSCost(true, false);}
                        }else{
                            increase=currentNode->GetMSW();
                        }
                    }
                    ...
                    if (diff<0) {
                        ...
                        if (children->size()==1) {
                            increase=-currentNode->GetEW()/BANDWIDTH;
                        }else{//children's size larger than 1
                            GetTwoLargestElementTypetwo(children,LargestNode, secondLargest);//no-increasing, communication counted
                            temp=currentNode->GetParallelPart()-secondLargest->GetMSCost(true, false);
                            if (temp>=0) {
                                increase=-currentNode->GetEW()/BANDWIDTH;
                            }else{increase=-temp-currentNode->GetEW()/BANDWIDTH;}
                        }
                    }
                }
                
                parent = currentNode->GetParent();
                while (parent->GetId()!=1) {//not the root node
...
//  #pragma omp parallel for
    for (unsigned int step=0; step<size; ++step) {
        ...
        if (tree->GetNode(currentQNode->GetothersideID())->IsBorken()==true) {//this subtree has not been merged yet
            children=currentQNode->GetChildren();
...
double ASAP(Ctree* tree, unsigned int num_processors, unsigned int depth){//depth should be at least 1
    ...
    while (num_processors>1) {//Breaking an edge a time
        ...
        i=0;
        while (i<depth-add_child_deepth) {//avoid insert nodes that are already in PQ, when depth is larger than 1
            ++i;
            buffer_size=children_buffer.size();
            while (buffer_size>0) {
                children=children_buffer.front()->GetChildren();
    ...
    *lastsubtree = false;
    ...
    if (idleP>1) {//has at least 2 idle processor
        ...
        while (!tempQue.empty()) {
            ...
            tempQue.pop_back();
            if (children->size()>1) {//has at least 2 children
...
void MemoryCheck(Ctree* tree, int* chstart, int*children, double const memory_size, io_method_t method){//chstart, children are not modified
